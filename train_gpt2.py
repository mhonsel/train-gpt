...
class MLP(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.gelu = nn.GELU(approximate='tanh')  # approximation is historic quirk
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module, 'NANOGPT_SCALE_INIT'):
                std *= (2 * self.config.n_layer) ** -0.5  # see gpt-2 paper
...
# set up large batch size still fitting in GPU memory
